...
- CREATE INDEX idx_messages_conv_time ON messages(conversation_id, client_timestamp ASC) WHERE deleted_at IS NULL; — truy vấn lịch sử chat theo thứ tự thời gian (FR-12), partial index loại trừ tin nhắn đã xóa. API dùng thêm `id` làm tie-breaker ổn định trong `ORDER BY` và cursor khi nhiều tin nhắn có cùng `client_timestamp`.
