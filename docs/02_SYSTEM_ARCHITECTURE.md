...
- Pipeline xử lý hiện tại (cuối Phase 2): raw frame → landmark extraction (MediaPipe) → direction filter → stateless one-vector arm solve → parent-local/rest-relative delta → safety constraint → hệ số blendshape + `AvatarPosePacketV1`. Elbow plane/pole, twist stabilization, partial-segment recovery và inferred elbow chưa triển khai.
...
child. Production hiện dừng ở one-vector parent-local solver của Phase 2. Elbow plane/pole,
swing–twist stabilization, partial recovery, palm twist và anatomical calibration chưa nằm
