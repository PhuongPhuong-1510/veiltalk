...
| Đường dẫn | Nội dung |
|---|---|
| `frontend/package.json` | Vite React/TypeScript project và scripts `dev`, `build`, `lint`, `preview`; F5-0 pin `onnxruntime-web@1.29.0` cho qualification Worker, chưa dùng trong production pipeline |
| `frontend/package-lock.json` | Khóa phiên bản dependency |
| `frontend/src/main.tsx` | Entry-point React |
| `frontend/src/App.tsx` | Router Phase 4: Splash/Welcome/Login/Register/Onboarding/Avatar Setup; nối auth lifecycle và Messaging WS; lazy-load route DEV `/dev/tracking`, `/dev/avatar-renderer` và `/dev/facial` chỉ khi `import.meta.env.DEV` |
...
| `frontend/src/lib/tracking/` | P4-T09: camera lifecycle, MediaPipe runtime dùng chung fileset, raw tracking contract local-only, mapper, cadence, metrics và test; các task cùng camera frame dùng chung source-sample timestamp, còn inference completion được đo riêng |
...
| `frontend/src/lib/avatar-motion/avatarPoseTypes.ts` | Discriminated union `AvatarPosePacketV1 | AvatarPosePacketV2`; V1 giữ legacy head semantic, V2 dùng calibrated rest-relative head và thêm hips/spine/upperChest. Cả hai chỉ chứa semantic rotation/expression, không chứa raw landmark/matrix |
...
| `frontend/src/lib/avatar-motion/avatarMotionProcessor.ts` | P4-T10 orchestration: Pose filter/solve/constraint/temporal, Hand matching/palm/twist, tracking epoch theo side và packet output. Phase 3B.4 tạo bản sao landmark chỉ khi Hand wrist 2D được reconstruct 3D thành công; raw frame gốc giữ nguyên; đổi nguồn Pose↔Hand kích hoạt reacquire blend. Palm-forward image-space đã match được cache ngắn hạn và cấp riêng cho elbow branch solver, kể cả khi Hand twist tắt. Phase 3B partial arm vẫn nghiệm thu upper/lower độc lập; hand twist gate theo lower. Face path nối F1 calibration → F2 eye/brow + F3 mouth → F4 mixer/dynamics; duplicate/loss/reacquire dùng một temporal owner |
| `frontend/src/lib/avatar-motion/facialNeutralCalibration.ts` / `facialNeutralCalibration.test.ts` | F1: thu median neutral bias local-only, dead zone và adaptive drift có neutral guard; nút calibration explicit chuyển sang manual collection để bias tự nhiên của webcam không làm bộ đếm kẹt 0/30. F4 đã nhận quyền sở hữu hold/return/reacquire amplitude, nên F1 không còn release expression riêng |
| `frontend/src/lib/avatar-motion/eyeBrowExpression.ts` / `eyeBrowExpression.test.ts` | F2: blink hai mắt phản hồi ngay với đường cong đạt full closure; unilateral spike phải qua confirmation 45 ms nhưng wink thật vẫn hoạt động; giữ squint/wide và brow trái/phải độc lập |
| `frontend/src/lib/avatar-motion/gazeObservation.ts` / `gazeSolver.ts` / `gazeTemporal.ts` | AR3-T01: eye-look observation đã neutral-calibrate → conjugate fusion/quality → asymmetric ellipse clamp → One Euro theo sample timestamp và hold/return; duplicate/reversed không promote, v1 không có convergence |
| `frontend/src/lib/avatar-motion/gazeEyelidCoupling.ts` / `gazeCinematic.ts` / `gazeMetrics.ts` | AR3-T02–T04: coupling chỉ tới target mí chuyên dụng đã audit; faithful mặc định và cinematic opt-in có attention/saccade/idle blink deterministic; metric scalar local-only cho jitter/clamp/reacquire, không giữ biometric trace |
| `frontend/src/lib/avatar-motion/upperBodyRigProfile.ts` / `upperBodyCalibration.ts` | AR4 infrastructure: immutable generation-bound profile; paired Face+shoulder calibration theo source timestamp, hips optional; mode cố định `shoulder-only` hoặc `full-torso` (≥80% full pairs), Pose yaw correction trước head-relative; V1 trong `idle/collecting`, V2 khi `calibrated` |
| `frontend/src/lib/avatar-motion/quaternionDistribution.ts` / `upperBodyComposer.ts` | AR4 quaternion log/exp, asymmetric ellipsoid clamp, fixed-order layer composition, final aggregate clamp và final parent-world hierarchy |
| `frontend/src/lib/avatar-motion/headNeckSolver.ts` / `torsoBasis.ts` / `torsoMotionSolver.ts` / `shoulderMotionSolver.ts` | AR4-T01–T03: Face/shoulder neutral-delta head-relative; shoulder-only basis giữ pitch thân unobserved ở neutral, full torso thêm bend; shrug vai–tai theo neutral axes không cần hông, differential protraction và bounded reach assist. Curl disabled/N/A |
| `frontend/src/lib/avatar-motion/upperBodyTemporal.ts` / `upperBodyLifeMotion.ts` / `upperBodyMetrics.ts` | AR4-T04/T05: timestamp filter, source hysteresis/transition, exact-neutral lifecycle, pause-safe breathing/speechChest/cinematic sway, jitter/response/solver metrics local-only |
| `frontend/src/lib/avatar-motion/mouthExpression.ts` / `mouthExpression.test.ts` | F3: mapper webcam mouth thuần và stateless; `safe01` chống nonfinite, validate config, jaw lấy max giữa blendshape và normalized inner-lip landmark aperture rồi vẫn tách closure/visible opening; round/width, five-vowel proportional normalization và corrective bất đối xứng; pucker/funnel curve được nén để mức vừa không thành cực đại |
| `frontend/src/lib/avatar-motion/mouthLandmarkGeometry.ts` / `mouthLandmarkGeometry.test.ts` | F3/F4 corrective: đo khe môi trong 13–14 theo pháp tuyến trục khóe 61–291, chuẩn hóa bằng bề rộng miệng và video aspect; bổ sung opening evidence khi MediaPipe `jawOpen` bỏ lỡ, chống scale/roll và reject landmark thiếu/nonfinite |
| `frontend/src/lib/avatar-motion/mouthPipelineTelemetry.ts` / `mouthPipelineTelemetry.test.ts` | F4 corrective diagnosis: cửa sổ peak 1 giây theo timestamp của fresh face sample, giữ scalar raw/calibrated → F3 → F4 desired/dynamic/final để panel 400 ms không bỏ lỡ xung nói nhanh; activity geometry không chứa smile hoặc vòng phản hồi viseme; reset theo facial lifecycle, không lưu ảnh/landmark và không đổi animation |
| `frontend/src/lib/avatar-motion/mouthSpeechCorrective.ts` / `mouthSpeechCorrective.test.ts` | F4 fast-speech corrective trước mixer: nonlinear mid-range lift chỉ trên năm viseme F3 hiện có, normalized-shape peak envelope decay 90 ms theo wall-clock, shape mới thay shape cũ không cộng nguyên âm, strong closure reset opening envelope, tracking gap/reset xóa state; không tăng global jaw, không tạo viseme từ silence/smile và Filter OFF bỏ temporal hold |
| `frontend/src/lib/avatar-motion/facialExpressionMixer.ts` / `facialExpressionMixer.test.ts` | F4 stateless pre/post projection: canonical closure, idempotent available-budget cho vowel/mắt, antagonist difference, budget tách lip-shape/vertical/corner/cheek/emotion và aggregate fallback không dùng full-face preset |
| `frontend/src/lib/avatar-motion/facialExpressionDynamics.ts` / `facialExpressionDynamics.test.ts` | F4 temporal owner duy nhất của expression: attack/release theo nhóm cơ, `dtMax=80ms`, gap rebase, Filter OFF vẫn cập nhật visible state, hold/smoothstep-return/reacquire không snap và lưu final post-mix chống hidden energy |
| `frontend/src/lib/avatar-motion/torsoBasis.ts` / `armFrameSolver.ts` | Phase 3A/AR4: per-segment direction validity, parallel transport và two-bone inferred elbow. Corrective 3B.4 quét 24 pole; AR4 cho upper arm nhận final animated shoulder target world để chest/vai không làm lệch arm world direction |
...
| `frontend/src/lib/avatar-motion/coordinateAdapter.ts` | P4-T10: vector conversion giữ `(x,y,z) → (x,-y,-z)`; facial quaternion đổi cả dấu X theo pitch convention avatar sau webcam corrective, normalize và chống zero/NaN. Yaw/roll vẫn cần manual gate riêng |
...
| `frontend/src/lib/avatar-renderer/modelLoader.ts` | Load GLTF/VRM, `rotateVRM0`, capture rig/finger/facial/gaze capability theo model generation, chống stale load và dispose tài nguyên |
| `frontend/src/lib/avatar-renderer/avatarRenderer.ts` | Tái tạo `restLocal × deltaLocal`; F4 expression và AR3 gaze đã temporal ở processor nên renderer không smooth hai đường này lần hai. Gaze adapter reset model cũ trước swap/dispose |
| `frontend/src/lib/avatar-renderer/gazeCapabilityAdapter.ts` / `gazeCapabilityAdapter.test.ts` | AR3-T01/T02: chọn độc quyền LookAt bone/expression, eye-bones hoặc unsupported. LookAt dùng range map thật; fallback đủ hai mắt suy local axes từ rest head/eye frame và áp `restLocal × deltaLocal`; partial/no capability no-op; xuất eyelid support chỉ từ exact verified/profile expression key |
| `frontend/src/lib/avatar-renderer/facialCapability.ts` / `facialCapability.test.ts` | Manifest bất biến phân biệt expression và gaze adapter/range/vertical-eyelid ownership; raw candidate không tự nâng thành production mapping |
| `docs/AR3_EYE_GAZE_ATTENTION_IMPLEMENTATION_PLAN.md` | Kế hoạch AR3 đã review; T01–T04 code/tooling complete, unified webcam/head-relative/đa-model manual gate pending |
| `docs/AR4_HEAD_NECK_TORSO_SHOULDER_IMPLEMENTATION_PLAN.md` | Kế hoạch toán học AR4-T01–T05 đã sửa theo review 8.5/10, chờ duyệt lần 2: Packet V2, temporal-before-hierarchy, aggregate clamp, neutral head-relative, torso/curl/offset proxy, ancestor-aware arm, shoulder/clavicle, pause-safe life motion và upper-body gate |
| `docs/AR4_T06_HYBRID_TORSO_LEAN_PLAN.md` | Contract và trạng thái AR4-T06: full-torso fore/aft angle, shoulder-only conservative proxy, camera-approach ambiguity gate, source lifecycle, head-relative recompute; code/automated gate hoàn tất, manual/performance còn chờ |
| `frontend/src/lib/avatar-motion/torsoLeanSolver.ts` / `torsoLeanSolver.test.ts` | AR4-T06: full shoulder–hip angle và shoulder-only depth/scale proxy có hysteresis/cap; xuất lean evidence để triệt riêng shoulder common drift do perspective, không triệt differential L/R |
| `frontend/src/lib/avatar-renderer/facialExpressionProfile.ts` / `facialExpressionProfile.test.ts` | F2–F4: profile raw morph VRoid theo fingerprint; model còn tên nhận dạng suffix, hai VRM 0 mất tên dùng allowlist key số. Map brow và các mouth morph có thật; model 2 không map raw 25/29 vì asset không có Close/Small độc lập và raw 25 Extra đụng vùng mắt. Smile closed/open dùng mouth Fun/Joy, frown dùng mouth Sorrow, không dùng preset `happy`. Raw eye Joy bị loại khỏi production vì webcam chứng minh nó đóng mắt khi blink=0; model số lạ không được suy đoán |
| `docs/F3_WEBCAM_MOUTH_CORE_MATH_PLAN.md` | Thiết kế F3 trước code: chuẩn hóa bằng chứng jaw/closure/round/width, công thức suy ra năm vowel webcam, normalization, capability strategy và test gate; audio/dynamics được giữ ngoài phạm vi F3 |
| `docs/F4_EXPRESSION_DYNAMICS_MIXER_MATH_PLAN.md` | Thiết kế và trạng thái F4 đã review: single-owner expression dynamics, attack/release theo nhóm cơ, fast visual lip closure, conflict projection idempotent, regional saturation budget, loss/reacquire không snap và test/webcam gate; code complete, chờ manual webcam |
| `docs/F5_AUDIO_WEBCAM_FUSION_MATH_PLAN.md` | Kế hoạch F5 đã review: provider descriptor + model/license/browser benchmark gate, AudioWorklet→Worker direct channel, VAD/semantic CTC confidence, causal zero-lookahead coarticulation, qW-aware webcam-authoritative fusion, privacy/lifecycle và automated/manual gate; F5-0 đã pin runtime nhưng chưa chọn model/viết audio pipeline |
| `docs/F6_CONVERSATIONAL_EXPRESSION_LAYER_MATH_PLAN.md` | Kế hoạch F6 chờ duyệt: faithful/lively mode, continuous observed intent, coupling má/mũi/mày có cap và capability gate, micro-motion deterministic chỉ trên secondary, neutral lock không trùng F1, không tự phát full-face preset và manual gate ba VRM |
| `docs/F5_0_AUDIO_MODEL_QUALIFICATION_REPORT.md` | Trạng thái F5-0: Uni2005 INT8 graph/checksum + preliminary Node/WASM benchmark, automated evidence và browser/FPS/license/Vietnamese gate còn mở |
| `frontend/src/lib/audio-lipsync/audioModelQualification.ts` / `.test.ts` | F5-0 pure contract/gate: provider descriptor, candidate artifact/license/probe, vocab parser, timing percentile, SHA/size/graph/p95/manual gate; không PCM |
| `frontend/src/lib/audio-lipsync/audioQualification.worker.ts` / `audioQualificationController.ts` | F5-0 DEV-only ORT Worker benchmark: local ArrayBuffer transfer, SHA-256, WASM/WebGPU session, synthetic tensor warmup/measurement, timeout/terminate; không nối microphone/facial pipeline |
| `frontend/src/components/dev/AudioQualificationPanel.tsx` / `audioQualificationPanel.css` | Panel trong `/dev/avatar-renderer` để chọn model+vocab local, chạy benchmark khi tracking/render hoạt động và xem gate/FPS trước→sau; không upload hoặc lưu file |
| `frontend/dev-assets/audio/candidates/` | Artifact qualification local-only đã bị `.gitignore`; hiện có Uni2005 INT8 + vocab/license/model card để chủ dự án chọn trong DEV panel, không thuộc production bundle hoặc model đã duyệt |
...
| `frontend/src/components/dev/AvatarRendererDevHarness.tsx` | Harness local-only tại `/dev/avatar-renderer`; chọn/reload một trong 5 VRM local, giữ model cũ tới khi model mới swap thành công và chỉ commit request mới nhất; đổi model reset rig/calibration/motion state; capability blocker và tên required bone bị thiếu được báo rõ; panel F1–F4 hiển thị calibration, eye-brow, mouth, final blink/squint/smile và dynamics/conflict/budget/lifecycle. Corrective fast-speech panel hiển thị current + peak 1 giây raw/calibrated/F3/F4 và target đã gửi VRM để baseline nói nhanh; freeze webcam, axes/vector helpers, hand-twist/bone-smoothing và collector fixture 3B.3; không upload landmark; route bị loại khỏi production build |
...
| `frontend/src/components/dev/FacialDevHarness.tsx` / `facialDevHarness.css` | F0 lab tại `/dev/facial`: close-up avatar, chọn model, thử độc lập preset/raw morph, xem bảng capability và tải manifest JSON local; không bật webcam, không upload và bị loại khỏi production build |
...
| `docs/11_AVATAR_EVOLUTION_ROADMAP.md` | Roadmap AR độc lập để ưu tiên nâng cấp avatar: Reliable → Contact → Interaction → Research; dùng mã task `AR*`, không thay thế hoặc đổi luồng `09_ROADMAP_AND_TASKS.md` |
...
| `frontend/vite.config.ts` | Vite React plugin và alias `@` → `src`; DEV server gửi COOP/COEP để F5-0 đo WASM multi-thread trong cross-origin-isolated context, chưa áp production deployment |
...
| P4-T10 / AR3 | IN PROGRESS | Arm/hand baseline giữ nguyên. AR3-T01–T04 code/tooling complete: semantic gaze model-independent, conditional eyelid, faithful/cinematic, LookAt/eye-bone adapter và local metrics. Manual smoke dẫn tới corrective giảm gaze response + 85% applied range; automated 68 files / 723 tests PASS, chờ webcam retest và unified đa-model gate |
...
| `frontend/src/lib/avatar-motion/` | Chuyển raw tracking thành packet V1 legacy hoặc V2 upper-body; sở hữu coordinate convention, rig-independent math, Pose/Hand/upper-body temporal state và diagnostics |
...
3. **Phase 3A — Anatomical arm-frame: IMPLEMENTED, MANUAL GATE PENDING.** Production solver
   ...
   normalized offset, projected previous/rest pole, pole filter và time-based hysteresis. Từ AR4, torso basis đồng thời cấp observed
   torso/head-relative/shoulder layer qua composer; arm nhận final animated parent. Processor phát rotation thật qua hold/return/recovery và idle identity; state tách
...
- `docs/AR4_T03_VERTICAL_SHRUG_EXTENSION_PLAN.md`: AR4-T03.1 đã implement current torso-up, head-tilt gate và
  full-state Packet V2 snapshot; automated gate PASS, manual/performance còn pending. Solver/scalar temporal nằm ở
  `avatar-motion/shoulderMotionSolver.ts` + `upperBodyTemporal.ts`; model adapter ở
  `avatar-renderer/modelLoader.ts` + `avatarRenderer.ts`; DEV telemetry ở `AvatarRendererDevHarness.tsx`.
  Shoulder translation dùng raw skinned bone sau `vrm.update()`; không gán position lên normalized shoulder vì
  three-vrm chỉ transfer normalized position của hips.
