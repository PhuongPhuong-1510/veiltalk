...
export class UpperBodyLifeMotion {
  ...
  update(input:LifeMotionInput,profile:UpperBodyRigProfileV1):UpperBodyLayer{
    ...
    const phase=2*Math.PI*this.continuousLifeTimeMs/BREATHING_CYCLE_MS;
    ...
    const amplitudes={chest:radians(.6)*LIFE_MOTION_GAIN,upperChest:radians(.8)*LIFE_MOTION_GAIN} as const;
    ...
    for(const name of ["leftShoulder","rightShoulder"] as const){const joint=profile.joints[name];if(joint)layer[name]=semanticRotationToLocal({x:0,y:0,z:(name==="leftShoulder"?1:-1)*breathing*radians(.25)*LIFE_MOTION_GAIN},joint);}
